...
const link = z.string().refine(isHttpUrl, 'Use an HTTP(S) URL without embedded credentials.');
...
const projects = defineCollection({
  ...
  schema: (context) => z.object({
    ...
    message: 'A hero image requires meaningful alt text.', path: ['heroAlt'],
  }).refine((data) => !data.githubUrl || data.visibility === 'public', {
    message: 'A source-code link requires explicitly public repository visibility.', path: ['githubUrl'],
  }),
});
...
const notes = defineCollection({
  ...
  schema: (context) => z.object({
    ...
    category: z.enum(['Architecture', 'AI Systems', 'Experiments', 'Engineering', 'Product', 'Building in Public', 'Lessons Learned']),
    tags: z.array(z.string().min(1)).default([]),
    relatedProjects: z.array(slug).default([]),
  }).refine((data) => !data.heroImage || Boolean(data.heroAlt), {
    message: 'A hero image requires meaningful alt text.', path: ['heroAlt'],
  }).refine((data) => !data.updated || data.updated >= data.date, {
    message: 'An update cannot precede publication.', path: ['updated'],
  }),
});
