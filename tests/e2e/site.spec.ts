...
for (const route of routes) {
  test(`${route} has semantics, metadata, working local assets, and no axe violations`, async ({ page }) => {
    const errors: string[] = [];
    ...
    await expect(page.locator('link[rel="canonical"]')).toHaveAttribute('href', `${site.origin}${withBase(route, site.base)}`);
...
test('navigation and footer point to real local pages', async ({ page, request }) => {
  await page.goto(withBase('/', site.base));
  const links = await page.locator('header nav a, footer nav a').evaluateAll((elements) => elements.map((element) => element.getAttribute('href') ?? ''));
...
test('core navigation remains usable without JavaScript', async ({ browser }) => {
...
test('320px reflow and reduced-motion styles remain usable', async ({ page }) => {
...
test('CV download and indexing follow verified content', async ({ page }) => {
  ...
  await expect(page.locator('meta[name="robots"]')).toHaveAttribute('content', !site.indexable || profile.experience.length === 0 ? 'noindex, nofollow' : 'index, follow');
});

test('drafts are absent from public pages and the sitemap', async ({ request }) => {
  ...
  const response = await request.get(withBase('/work/draft-fixture/', site.base));
  expect(response.status()).toBe(404);
});

test('capture homepage for human visual review, without approving a baseline', async ({ page }, testInfo) => {
  await page.goto(withBase('/', site.base));
  await page.evaluate(() => document.fonts.ready);
  await page.screenshot({ path: testInfo.outputPath('home-review.png'), fullPage: true });
});

test('fixture build renders real MDX templates, hides drafts, and links only published related work', async ({ page, request }) => {
  test.skip(!site.testContent, 'Runs only against the isolated test-dist build.');
  await page.goto(withBase('/work/public-fixture/', site.base));
  await expect(page.getByRole('heading', { level: 1 })).toHaveText('Test fixture — public case study');
  await expect(page.getByRole('heading', { name: 'Architecture', exact: true })).toBeVisible();
  await expect(page.getByRole('complementary', { name: 'Decision' })).toBeVisible();
  expect((await request.get(withBase('/work/draft-fixture/', site.base))).status()).toBe(404);
  expect((await request.get(withBase('/notes/future-fixture/', site.base))).status()).toBe(404);
  ...
  await expect(page.getByRole('link', { name: 'Test fixture — public case study →' })).toBeVisible();
